...
export function formatMedicalHistory(data: MedicalHistory): string {
  const formatPreExisting = () => {
    if (!data?.preExistingConditions?.length) return "No pre-existing conditions reported.";
  ...
  const formatInjury = () => {
    if (!data?.injury) return "No injury details reported.";
    return `
  ...
  const formatTreatments = () => {
    if (!data?.currentTreatments?.length) return "No current treatments reported.";
  ...
  const formatMedications = () => {
    if (!data?.currentMedications?.length) return "No current medications reported.";
...
export function preparePrompt(data: MedicalHistory, type: keyof typeof PROMPT_TEMPLATES = 'standard'): string {
  ...
  const template = PROMPT_TEMPLATES[type] || PROMPT_TEMPLATES.standard;
