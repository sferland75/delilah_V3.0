...
describe('Medical History Prompts', () => {
  describe('formatMedicalHistory', () => {
    ...
    it('handles missing optional fields gracefully', () => {
      const partialHistory: MedicalHistory = {
        ...mockMedicalHistory,
        preExistingConditions: [{
          condition: 'Test',
          status: 'active',
      ...
      const formatted = formatMedicalHistory(partialHistory);
      expect(formatted).toContain('Not specified');
    });

    it('handles special characters in text fields', () => {
      const specialHistory: MedicalHistory = {
        ...mockMedicalHistory,
        preExistingConditions: [{
          condition: 'Test & Special < > " \' Characters',
          status: 'active',
          details: 'Details with & < > " \' special chars'
        }]
      };
      const formatted = formatMedicalHistory(specialHistory);
      expect(formatted).toContain('Test & Special');
  ...
  describe('preparePrompt', () => {
    ...
    it('handles unknown prompt types gracefully', () => {
      // @ts-ignore - Testing invalid type
      const prompt = preparePrompt(mockMedicalHistory, 'invalid');
      expect(prompt).toContain('comprehensive medical history');
    });

    it('sanitizes medical history data in prompt', () => {
      const specialHistory: MedicalHistory = {
        ...mockMedicalHistory,
        preExistingConditions: [{
          condition: 'Test & Special < > " \' Characters',
      ...
      const prompt = preparePrompt(specialHistory);
      expect(prompt).toContain('Test & Special');
