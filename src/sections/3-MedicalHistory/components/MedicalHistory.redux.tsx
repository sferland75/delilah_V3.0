...
export function MedicalHistoryRedux() {
  ...
  // Map Redux data to form and update the form only once on initial load
  useEffect(() => {
    // Get the medical history data from Redux
    const medicalHistoryData = currentData?.medicalHistory || {};
    ...
    if (dataChanged && !initialLoadRef.current && Object.keys(medicalHistoryData).length > 0) {
      try {
        console.log("MedicalHistory Redux - Context data detected, mapping to form");
        ...
        console.error("MedicalHistory Redux - Error updating form from Redux:", error);
      }
    } else if (!initialLoadRef.current) {
      console.log("MedicalHistory Redux - No Redux data or empty object");
      // Still mark as initialized to prevent further checks
      initialLoadRef.current = true;
    }
  }, [currentData?.medicalHistory, form]); // Only depend on medical history changes
  ...
  const onSubmit = async (formData) => {
    try {
      ...
      dispatch(addToast({
        ...
        description: "Medical History information has been saved successfully!",
        type: "success"
      }));
      
    } catch (error) {
      console.error("MedicalHistory Redux - Error preparing data for Redux update:", error);
      
      dispatch(addToast({
        title: "Save Failed",
        description: "Error saving Medical History. Please try again.",
        type: "error"
      }));
    }
  };
      ...
      try {
        ...
        if (importedData) {
          ...
          dispatch(addToast({
            ...
            description: "Medical History data imported successfully!",
            type: "success"
          }));
        }
      } catch (error) {
  ...
  const handleDownloadJson = () => {
    ...
    dispatch(addToast({
      ...
      description: "Medical History data exported successfully!",
      type: "success"
    }));
  };
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="w-full">
          ...
          <div className="flex justify-end space-x-2 mt-4">
            ...
            <Button 
              ...
              disabled={saveStatus === 'loading'}
            >
              {saveStatus === 'loading' ? 'Saving...' : 'Save Medical History'}
            </Button>
          </div>
        </form>
      </Form>
