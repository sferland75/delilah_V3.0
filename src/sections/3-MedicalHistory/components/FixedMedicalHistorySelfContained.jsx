...
// IMPORTANT: Removed the Form and FormProvider imports which may be causing issues

export function FixedMedicalHistorySelfContained() {
  ...
  // Simple form without zod validation and WITHOUT FormProvider
  const form = useForm({
    ...
    if (data?.medicalHistory) {
      try {
        ...
        if (data.medicalHistory.treatmentHistory?.rehabilitationServices) {
          formData.data.currentTreatments = data.medicalHistory.treatmentHistory.rehabilitationServices.map(t => ({
            type: t.type || '',
            provider: t.provider || '',
            facility: t.facility || '',
        ...
        if (data.medicalHistory.pastMedicalHistory?.medications) {
          formData.data.currentMedications = data.medicalHistory.pastMedicalHistory.medications.map(m => ({
            name: m.name || '',
            dosage: m.dosage || '',
    ...
    try {
      ...
      const medicalHistoryData = {
        pastMedicalHistory: {
          ...
          allergies: [],
          medications: formData.data.currentMedications.map(m => ({
            name: m.name,
            dosage: m.dosage,
            frequency: m.frequency,
            reason: m.prescribedFor,
  ...
  // Pre-existing conditions section - completely self-contained
  ...
  // Injury details section - completely self-contained
  ...
  // Treatment section - completely self-contained
  ...
  // Medications section - completely self-contained
  ...
  // Using traditional HTML tabs instead of UI library components to reduce dependencies
  return (
    <div className="space-y-6">
      <div className="mb-4">
      ...
      <form onSubmit={onSubmit} className="w-full">
        <div className="w-full border rounded-md">
          <div className="grid w-full grid-cols-4 p-0 h-auto border-b">
            <button
              type="button"
              className={`py-2 rounded-none ${activeTab === "preExisting" 
            ...
            <button
              type="button"
              className={`py-2 rounded-none ${activeTab === "injury" 
                ? "border-b-2 border-blue-600 text-blue-600" 
                : "border-b border-gray-200 text-gray-600"}`}
            ...
            <button
              type="button"
              className={`py-2 rounded-none ${activeTab === "treatment" 
                ? "border-b-2 border-blue-600 text-blue-600" 
                : "border-b border-gray-200 text-gray-600"}`}
            ...
            <button
              type="button"
              className={`py-2 rounded-none ${activeTab === "medications" 
                ? "border-b-2 border-blue-600 text-blue-600" 
                : "border-b border-gray-200 text-gray-600"}`}
...
export default FixedMedicalHistorySelfContained;