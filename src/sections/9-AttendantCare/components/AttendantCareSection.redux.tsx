...
export function AttendantCareSectionRedux({ initialData, onDataChange }: AttendantCareSectionProps) {
  ...
  const contextData = currentData.attendantCare || {};
  
  const [showSummary, setShowSummary] = React.useState(false);
  const [currentCalculations, setCurrentCalculations] = React.useState<any>(null);
  const [dataLoaded, setDataLoaded] = React.useState(false);

  console.log("Initial attendant care Redux data:", contextData);
  ...
  const onSubmit = useCallback(async (formData: AttendantCareFormData) => {
    try {
      ...
      await dispatch(updateSectionThunk({
        sectionName: 'attendantCare',
        sectionData: attendantCareData
      }));
      ...
      if (saveCurrentAssessmentThunk.fulfilled.match(saveResult)) {
        dispatch(addToast({
          title: "Attendant Care Saved",
          description: "Attendant care assessment has been saved successfully.",
          type: "success"
        }));
      } else {
        dispatch(addToast({
          title: "Save Failed",
          description: "There was an error saving the attendant care assessment.",
          type: "error"
        }));
      }
    } catch (error) {
      ...
      <form onSubmit={form.handleSubmit(onSubmit)}>
        ...
        <div className="flex justify-between pt-4 border-t mt-4">
          ...
          <Button 
            ...
            disabled={saveStatus === 'loading'}
          >
            {saveStatus === 'loading' ? 'Saving...' : 'Save Attendant Care Assessment'}
          </Button>
        </div>
      </form>
