...
describe('Level1Care Component', () => {
  beforeEach(() => {
    // Mock all category icons
    Object.values(careCategories.level1).forEach(category => {
  ...
  it('renders the component title and description', () => {
    render(
      <FormWrapper>
        <Level1Care form={useForm()} />
      </FormWrapper>
    );
    
    // Check for the title
    expect(screen.getByText('Level 1 Attendant Care')).toBeInTheDocument();
    
    // Check for the description
    expect(screen.getByText(LEVEL_DESCRIPTIONS.LEVEL_1)).toBeInTheDocument();
  });
  ...
  it('renders the correct number of care activities', () => {
    ...
    const totalActivities = Object.values(careCategories.level1).reduce(
  ...
  it('passes the correct path to CareActivity components', () => {
    ...
    Object.entries(careCategories.level1).forEach(([categoryKey, category]) => {
      category.items.forEach(item => {
        const expectedPath = `level1.${categoryKey}.${item.id}`;
