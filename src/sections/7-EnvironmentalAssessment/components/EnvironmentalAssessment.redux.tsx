...
export function EnvironmentalAssessmentRedux() {
  ...
  const contextData = currentData.environmentalAssessment || {};
  ...
  useEffect(() => {
    try {
      if (contextData && Object.keys(contextData).length > 0 && !initialLoadRef.current) {
        console.log("Environmental assessment Redux data detected, mapping to form");
  ...
  const { persistForm } = useFormPersistence(form, 'environmental-assessment');
  ...
  const onSubmit = async (formData: FormState) => {
    try {
      ...
      await dispatch(updateSectionThunk({
        sectionName: 'environmentalAssessment',
        sectionData: environmentalAssessmentData
      }));
      ...
      if (saveCurrentAssessmentThunk.fulfilled.match(saveResult)) {
        dispatch(addToast({
          title: "Environmental Assessment Saved",
          description: "Environmental assessment has been saved successfully.",
          type: "success"
        }));
      } else {
        dispatch(addToast({
          title: "Save Failed",
          description: "There was an error saving the environmental assessment.",
          type: "error"
        }));
      }
    } catch (error) {
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="w-full">
          ...
          <div className="flex justify-end space-x-2 mt-4">
            ...
            <Button 
              ...
              disabled={saveStatus === 'loading'}
            >
              {saveStatus === 'loading' ? 'Saving...' : 'Save Environmental Assessment'}
            </Button>
          </div>
        </form>
      </Form>
