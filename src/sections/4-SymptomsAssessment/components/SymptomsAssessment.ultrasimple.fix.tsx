...
export function SymptomsAssessment() {
  console.log('FIXED SymptomsAssessment component is executing');
  ...
  useEffect(() => {
    console.log('SymptomsAssessment component mounted');
    
    // Return cleanup function
    return () => {
      console.log('SymptomsAssessment component unmounting');
    };
  }, []);

  // Update form when context data changes
  useEffect(() => {
    console.log('Context data changed in useEffect');
    if (contextData && Object.keys(contextData).length > 0) {
      const formData = mapContextDataToForm();
      form.reset(formData);
      console.log('Form data updated from context');
    }
  }, [contextData]);
  ...
  const onSubmit = (formData: Symptoms) => {
    ...
    console.log('Mapped data for context:', symptomsData);
  ...
  const DebugInfo = () => (
    <div className="bg-red-100 p-4 mb-4 rounded border border-red-300">
      ...
      <div className="text-sm text-red-700">
        ...
        <p>Symptoms Assessment Data: {contextData && Object.keys(contextData).length > 0 ? 'Present' : 'Not Present'}</p>
      </div>
    </div>
  );
