...
export function SymptomsAssessmentIntegrated() {
  ...
  // Map context data to form structure with error handling - Using useMemo to prevent recalculation on every render
  const initialFormValues = useMemo(() => {
    try {
      ...
      // Only process if we actually have context data
  ...
  // Update form when context data changes and set dataLoaded flag
  useEffect(() => {
    try {
      if (contextData && Object.keys(contextData).length > 0) {
        form.reset(initialFormValues);
        
        // Check if we received any valid data to show the "Data Loaded" alert
        const hasPhysical = contextData.physicalSymptoms && contextData.physicalSymptoms.length > 0;
        const hasCognitive = contextData.cognitiveSymptoms && contextData.cognitiveSymptoms.length > 0;
        const hasEmotional = contextData.emotionalSymptoms && contextData.emotionalSymptoms.length > 0;
        const hasNotes = contextData.generalNotes && contextData.generalNotes.trim() !== '';
        
        if (hasPhysical || hasCognitive || hasEmotional || hasNotes) {
          setDataLoaded(true);
        }
      }
    } catch (error) {
  ...
  }, [contextData, initialFormValues, form]);
