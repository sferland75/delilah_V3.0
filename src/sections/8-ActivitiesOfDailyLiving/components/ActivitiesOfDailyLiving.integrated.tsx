...
export function ActivitiesOfDailyLivingIntegrated() {
  const { data, updateSection } = useAssessment();
  const contextData = data.activitiesDailyLiving || {};
  
  console.log("Initial ADL context data:", contextData);
  ...
  const mapContextDataToForm = (): ADLData => {
    try {
      ...
      console.log("Mapping ADL context data:", contextData);
  ...
  useEffect(() => {
    try {
      if (contextData && Object.keys(contextData).length > 0) {
        console.log("ADL context data changed:", contextData);
