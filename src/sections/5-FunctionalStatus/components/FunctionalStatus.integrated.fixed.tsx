...
// Import our fixed components instead of the problematic ones
import { RangeOfMotion } from './rom/EnhancedROM'; // This should already be the fixed ROM component
import { ManualMuscle } from './ManualMuscle.fixed';
import { BergBalance } from './BergBalance.fixed';
import { PosturalTolerances } from './PosturalTolerances.fixed';
import { TransfersAssessment } from './TransfersAssessment.fixed';
...
export function FunctionalStatusIntegrated() {
  ...
  const getSittingToleranceFromForm = (formData: FormState): string => {
    try {
      ...
      return `${sitting.toleranceLevel || 'Not assessed'}${sitting.duration ? ', ' + sitting.duration + ' ' + sitting.unit : ''}`;
    } catch (error) {
  ...
  const getStandingToleranceFromForm = (formData: FormState): string => {
    try {
      ...
      return `${standing.toleranceLevel || 'Not assessed'}${standing.duration ? ', ' + standing.duration + ' ' + standing.unit : ''}`;
    } catch (error) {
  ...
  const getWalkingToleranceFromForm = (formData: FormState): string => {
    try {
      const walking = formData.data.posturalTolerances.dynamic.walking;
      return `${walking.toleranceLevel || 'Not assessed'}${walking.duration ? ', ' + walking.duration + ' ' + walking.unit : ''}`;
    } catch (error) {
  ...
  const getBergBalanceScoreFromForm = (formData: FormState): number => {
    try {
      ...
      if (berg.sittingToStanding?.score) totalScore += berg.sittingToStanding.score;
      if (berg.standingUnsupported?.score) totalScore += berg.standingUnsupported.score;
      if (berg.sittingUnsupported?.score) totalScore += berg.sittingUnsupported.score;
      if (berg.standingToSitting?.score) totalScore += berg.standingToSitting.score;
      if (berg.transfers?.score) totalScore += berg.transfers.score;
      if (berg.standingWithEyesClosed?.score) totalScore += berg.standingWithEyesClosed.score;
      if (berg.standingWithFeetTogether?.score) totalScore += berg.standingWithFeetTogether.score;
      if (berg.reachingForwardWithOutstretchedArm?.score) totalScore += berg.reachingForwardWithOutstretchedArm.score;
      if (berg.pickingUpObject?.score) totalScore += berg.pickingUpObject.score;
      if (berg.turningToLookBehind?.score) totalScore += berg.turningToLookBehind.score;
      if (berg.turning360Degrees?.score) totalScore += berg.turning360Degrees.score;
      if (berg.placingAlternateFoot?.score) totalScore += berg.placingAlternateFoot.score;
      if (berg.standingWithOneFootAhead?.score) totalScore += berg.standingWithOneFootAhead.score;
      if (berg.standingOnOneLeg?.score) totalScore += berg.standingOnOneLeg.score;
