...
export function FunctionalStatusRedux() {
  ...
  const contextData = currentData.functionalStatus || {};
  const [dataLoaded, setDataLoaded] = useState(false);
  const initialLoadRef = useRef(false);
  
  console.log("Initial functional status Redux data:", contextData);
  ...
  const mapContextDataToForm = (): FormState => {
    try {
      ...
      console.log("Mapping functional status Redux data:", contextData);
  ...
  useEffect(() => {
    try {
      if (contextData && Object.keys(contextData).length > 0 && !initialLoadRef.current) {
        console.log("Functional status Redux data detected, mapping to form");
  ...
  const { persistForm } = useFormPersistence(form, 'functional-status');
  ...
  const onSubmit = async (formData: FormState) => {
    try {
      ...
      await dispatch(updateSectionThunk({
        sectionName: 'functionalStatus',
        sectionData: functionalStatusData
      }));
      ...
      if (saveCurrentAssessmentThunk.fulfilled.match(saveResult)) {
        dispatch(addToast({
          ...
          description: "Functional status assessment has been saved successfully.",
          type: "success"
        }));
      } else {
        dispatch(addToast({
          title: "Save Failed",
          description: "There was an error saving the functional status assessment.",
          type: "error"
        }));
      }
    } catch (error) {
  ...
  const getStandingBalanceFromForm = (formData: FormState): string => {
    try {
      ...
      if (berg.standingUnsupported && berg.standingUnsupported.score === 4) {
        return "Good";
      } else if (berg.standingUnsupported && berg.standingUnsupported.score === 3) {
        return "Fair";
      } else if (berg.standingUnsupported && berg.standingUnsupported.score <= 2) {
        return "Poor";
      }
  ...
  const getSittingBalanceFromForm = (formData: FormState): string => {
    try {
      ...
      if (berg.sittingUnsupported && berg.sittingUnsupported.score === 4) {
        return "Good";
      } else if (berg.sittingUnsupported && berg.sittingUnsupported.score === 3) {
        return "Fair";
      } else if (berg.sittingUnsupported && berg.sittingUnsupported.score <= 2) {
        return "Poor";
      }
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Form wrapper - using div instead of form to avoid nesting forms */}
      <div className="w-full">
        {/* React Hook Form FormProvider wrapped around individual tab contents */}
